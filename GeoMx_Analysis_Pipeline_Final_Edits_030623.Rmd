...
```{r loading packages}
library_list<- c( "NanoStringNCTools", "GeomxTools", "EnvStats", "ggiraph", "SpatialDecon", "reshape" , "reshape2", "knitr", "dplyr", "ggplot2",  "ggforce","cowplot", "scales", "umap", "Rtsne", "pheatmap", "ggrepel", "rentrez", "RColorBrewer", "Polychrome", "plyr",  "openxlsx", "psych", "kableExtra", "GGally", "magick", "circlize", "pals", "ggupset", "tidyr", "ggpubr", "ggprism", "Biobase", "FactoMineR", "svglite", "corrplot", "pheatmap", "readr", "qusage", "GSVA", "GSEABase", "ggcorrplot","ggbreak","rentrez", "stringr", "writexl", "readxl")
...
outdir <- paste0("qc", date_tag)
object_dir <- file.path(outdir, "R_objects")
qc_dir <- file.path("C:/Users/Thomas.Goralski/Documents/qc")
...
if(file.exists(file.path(object_dir, "probe_data.RDS"))){
 ...
 SampleAnnotationFile <-                                                 
  dir(file.path(datadir, fsep="\\" ), pattern = "File_plate_merged.xlsx",            #specify path and pattern to identify annotation file. ***Make sure it is not opne or else excel creates a temporary save of it, leading to 2 paths
      full.names = TRUE, recursive = TRUE)
  ...
  probe_data <-
    readNanoStringGeoMxSet(dccFiles = DCCFiles,                           #This loads the data in Nanostrings s4 data structure
                           ...
                           phenoDataSheet = "AnnotationFile_plate_merged",      #full name of annotation file
...
*************************
What needs to happen here. In probe data, get average exp for each aoi's neg probes. set any value ebelow that as zero in that segment. Etc. 

```{r}
#set all values above max neg probe detection to 0
# ind<-which(rownames(target_data)=="NegProbe-WTX")
#  min<-as.vector(target_data@assayData$exprs[ind,])
# min
# min_value<-max(min)
# sub_threshold<-which(probe_data@assayData$exprs<min_value)


# neg_subset <- NanoStringNCTools::subset(probe_data, CodeClass == "Negative")
# neg_median<-apply(neg_subset@assayData$exprs,2,median)
# <- assayDataApply(probe_data, 2, FUN = function(x) , elt = "exprs")

#neg_probe<-aggregateCounts(probe_data,'ngeoMean')
#ind<-which(rownames(target_data1)=="NegProbe-WTX")

 #min<-as.vector(target_data1@assayData$exprs[ind,])
#dat<-t(apply(dat, 1, FUN =function(x) x - min))

#assayDataElement(object=probe_data, elt = "exprs")[sub_threshold]<-0
```
...
ind1<-which(probe_data@phenoData@data$region=="CPu")
ind2<-which(probe_data@phenoData@data$region=="AI")
ind3<-which(probe_data@phenoData@data$layer=="2")
ind_sub<-c(ind1,ind2,ind3)
...
run_qc(Dataset = probe_data[,-ind_sub], Parameters = QC_params, segment_id = segment)
...
```{r}
#subtract out max value for Neun negative probes then remove negative probes
neun_sub<-which(target_data@phenoData@data$segment=="NeuN")
#first we remove any smaple wth high negative probe detection
ind<-which(rownames(target_data)=="NegProbe-WTX")
 
min<-as.vector(target_data@assayData$log_q[ind,neun_sub])
minmin
min_value<-max(min)
sub_threshold<-which(target_data@assayData$log_q<min_value)
assayDataElement(object=target_data, elt = "log_q")[sub_threshold]<-0
...
More subsetting of negative...not necessary
```{r}
min2<-target_data@assayData$exprs[ind,]
min2
min2<-melt(min2)
min2$segment<-target_data@phenoData@data$segment
min2$area<-target_data@phenoData@data$area
min2$nuclei<-target_data@phenoData@data$AOINucleiCount
View(min2)
ggplot(min2, aes(x=segment, y=value))+
  geom_boxplot()
ggscatter(min2, x = "area", y = "value", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson", fill = "segment")



min3<-target_data@assayData$log_q[ind,]
min3<-melt(min3)
min3$segment<-target_data@phenoData@data$segment
min3$area<-target_data@phenoData@data$area
ggplot(min3, aes(x=segment, y=value))+
  geom_boxplot()



sub<-which(min>0.06)
target_data1<-target_data[,-sub]
#subset out all data less then negative probe detection

dat<-target_data1@assayData$exprs
...
dat1<-dat[-apply(dat, 1, min) < 0, ]

sub1<-match(rownames(target_data),rownames(dat1))
target_data2<-target_data1[sub1,]
...
Subsetting negatives...not necessary
```{r}
neg_subset <- NanoStringNCTools::subset(probe_data, CodeClass == "Negative")


neg_probe<-neg_subset@assayData$exprs
colnames(neg_probe)<-neg_subset@phenoData@data$segment  
                                
neg_melt<-melt(neg_probe)
  

neg_melt$segment<-neg_melt$Var2


neg_melt$area<-neg_subset@phenoData@data$area
neg_melt$nuclei<-neg_subset@phenoData@data$AOINucleiCount
...
for(i in unique(melt$Var1)) {
  vec1<-which(melt$Var1== i)
p <-ggplot(melt[vec1,], aes(x=Var1, y=value))+
...
for(i in unique(melt$Var1)){
  vec1<-which(melt$Var1== i)
p<-ggscatter(melt[vec1,], x = "area", y = "value", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson", fill = "segment")
print(p)
...
for(i in unique(melt$Var1)){
  vec1<-which(melt$Var1== i)
p<-ggscatter(melt[vec1,], x = "nuclei", y = "value", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson", fill = "segment")
print(p)
}
